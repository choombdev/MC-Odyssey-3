// This script was made by choombdev for Odyssey 3, reusing it without explicit permission is not allowed.
onEvent('recipes', event => {

    // Replacing all Cast Iron Ingots with ones from Big Cannons
    event.replaceInput({}, 'createdeco:cast_iron_ingot', 'createbigcannons:cast_iron_ingot')
    event.replaceInput({}, 'createdeco:cast_iron_nugget', 'createbigcannons:cast_iron_nugget')

    event.replaceOutput({}, 'createdeco:cast_iron_nugget', 'createbigcannons:cast_iron_nugget')
    event.replaceOutput({}, 'createdeco:cast_iron_ingot', 'createbigcannons:cast_iron_ingot')

    // Adding a recipe for the Create Deco sheet
    event.recipes.createPressing('createdeco:cast_iron_sheet', 'createbigcannons:cast_iron_ingot')
})

